...
func cloneGraph(root *Node) *Node {
	...
	for len(stack) > 0 {
		...
		if _, exists := dict[node]; !exists {
			dict[node] = &Node{Val: node.Val}
		}
		for _, neighbor := range node.Neighbors {
			if _, exists := dict[neighbor]; !exists {
				dict[neighbor] = &Node{Val: neighbor.Val}
				stack = append(stack, neighbor)
			}
		}
	}
	for oldNode, newNode := range dict {
		for _, neighbor := range oldNode.Neighbors {
			newNode.Neighbors = append(newNode.Neighbors, dict[neighbor])
		}
	}
	return dict[root]
}
